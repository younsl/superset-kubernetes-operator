...
.PHONY: check-license
check-license: ## Check Apache license headers using Apache Rat.
...
clean: ## Remove build artifacts, downloaded tools, and test cache.
	...
	rm -rf $(LOCALBIN) dist cover.out Dockerfile.cross
	go clean -cache -testcache

.PHONY: manifests
manifests: controller-gen ## Generate ClusterRole and CustomResourceDefinition objects.
	$(CONTROLLER_GEN) rbac:roleName=manager-role crd:maxDescLen=0 paths="./..." output:crd:artifacts:config=config/crd/bases
...
.PHONY: test-unit
test-unit: manifests generate fmt vet ## Run unit tests (no envtest or cluster required).
	go test $$(go list ./... | grep -v /e2e) -coverprofile cover_ut.out
